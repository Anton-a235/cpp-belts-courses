    ...
    const vector<string> &expected)
{
    SearchServer srv;
    {
        string joined_docs = Join('\n', docs);
        LOG_DURATION("UpdateDocumentBase 50000 times");
        for (int i = 0; i < 50000; i++)
        {
            istringstream docs_input(joined_docs);
            srv.UpdateDocumentBase(docs_input);
        }
    }
    string result;
    {
        string joined_queries = Join('\n', queries);
        LOG_DURATION("AddQueriesStream 50000 times");
        for (int i = 0; i < 50000; i++)
        {
            istringstream queries_input(joined_queries);
            ostringstream queries_output;
            srv.AddQueriesStream(queries_input, queries_output);
            if (i == 0)
            {
                result = queries_output.str();
            }
        }
    }
