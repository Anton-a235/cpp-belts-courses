...
struct AddressHasher
{
    size_t operator()(const Address &a) const
    {
        ...
        return hash<string>{}(a.city) * x * x +
               hash<string>{}(a.street) * x +
               hash<int>{}(a.building);
    }
};
...
void TestDistribution()
{
    ...
    {
        Person person;
        person.name = WORDS[static_cast<size_t>(word_dist(gen))];
        person.height = height_dist(gen);
        person.weight = weight_dist(gen) * 0.5;
        person.address.city = WORDS[static_cast<size_t>(word_dist(gen))];
        person.address.street = WORDS[static_cast<size_t>(word_dist(gen))];
