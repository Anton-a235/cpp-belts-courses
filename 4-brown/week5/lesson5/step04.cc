...
using Budget = std::map<Date, BudgetStats>;
...
public:
    ...
    Result process(Budget& budget) const override
    {
        ...
        for (Date cur_day = from_; cur_day <= to_; cur_day = ++cur_day)
            budget[cur_day].earned += earn_per_day;
...
public:
    ...
    Result process(Budget& budget) const override
    {
        ...
        std::for_each(from_it,
                      ...
                      [value = value_](auto& p)
                      {
                          p.second.earned *= (1.0 - value / 100.0);
                      });
...
public:
    ...
    Spend() = delete;
    ~Spend() override = default;

    Result process(Budget& budget) const override
    {
        const auto days = Date::days_count(from_, to_);
        const double spend_per_day = value_ / static_cast<double>(days);

        for (Date cur_day = from_; cur_day <= to_; cur_day = ++cur_day)
            budget[cur_day].spent += spend_per_day;
...
bool requires_value(Type request)
{
    // clang-format off
    static const std::unordered_set<Type> set
    {
        Type::EARN,
        Type::PAY_TAX,
        Type::SPEND
...
public:
    ...
    RequestPtr build()
    {
        ...
        if (!value_ && requires_value(*type_))
        ...
        case Type::PAY_TAX:
            return std::make_unique<PayTax>(std::move(from_), std::move(to_), *value_);
        case Type::SPEND:
            return std::make_unique<Spend>(std::move(from_), std::move(to_), *value_);
        default:
            return nullptr;
        }
    }
...
RequestPtr read_one(std::istream& is)
{
    ...
    if (type = Requests::type_from_command(command); Requests::requires_value(type))
        if (!(is >> value))
            return nullptr;
